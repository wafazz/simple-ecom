...
9a. **A courier service is chosen at BOOK time, never read off the order.** `orders.courier_service_id` records what the *customer* was quoted — a weight-table id since REQ-006. Sending it to EasyParcel would be rejected. `ShipmentPayload::for()` takes the service as a required argument.
9b. **An order carrying an AWB can never be booked again.** Enforced in `Order::canBookShipment()`, in the bulk controller's own filter, and structurally by `UNIQUE(shipments.order_id)` — a stale page is not a defence.

6a. **Delivery is DERIVED, never chosen.** `ShippingRate::quoteFor()` takes the state from the validated address and the weight from the cart. No `shipping_service_id`, no posted fee, nothing from the browser reaches the figure — there is no longer anything for the server to "match" against.
...
| 2026-08-27 | **OQ-11 CLOSED — ToyyibPay verified against the official reference.** All `getBillTransactions` field names confirmed; amount confirmed as decimal ringgit. Fixed two defects the docs exposed: `billName`/`billDescription` were sending hyphens the gateway does not accept, and `getBillTransactions` was being sent an undocumented `userSecretKey`. Implemented the documented callback MD5 hash check, correcting assumption 8 which had claimed there was no signature | `app/Services/ToyyibPayService.php`, `app/Http/Controllers/PaymentController.php`, `config/services.php`, `Planning.md` §11.A.6–7 | Iris |
...
| 2026-08-27 | **Fix: bulk Print AWB opened only one label.** Two causes, both in the page. `window.open()` was passed a features string (`'noopener'`), which asks for a popup WINDOW rather than a tab — popups are blocked far more aggressively, and the argument also makes the call return `null` by spec, so nothing could detect the failure. And nothing counted how many actually opened, so nine blocked labels looked like success. Now: opened as tabs, blocked ones counted and reported with what to do about it, opened ones ticked so a part-finished batch is resumable, plus a Copy-all-links fallback that no blocker can touch. The server side was never wrong — every selected label was always listed, and a test now pins that. **There is no bulk-label endpoint in the EasyParcel API**, so a single combined PDF is not available to build | `resources/views/admin/orders/labels.blade.php`, `tests/Feature/Admin/BulkShipmentTest.php` | Iris |
| 2026-08-27 | **Courier service and AWB on the order list.** Shown on Processing, In Delivery, Completed and Returned — and on any page where a shipment exists. A `shipments.service_name` column was added because neither existing field answers "which service did we buy?": `service_id` is an EasyParcel identifier and `courier_service` is null in EasyParcel's own success example. It is resolved once per batch from the quotation, server-side, never from the form. Also fixed a misleading row on the order detail: it labelled `orders.courier_name` as "Courier", which since REQ-006 holds the weight-table label "Standard Delivery" rather than a carrier — the charged delivery and the booked courier are now shown as the separate things they are | `database/migrations/2026_08_27_200000_*`, `app/Models/Shipment.php`, `app/Services/ShipmentBookingService.php`, `app/Http/Controllers/Admin/ShipmentController.php`, `resources/views/admin/orders/{index,show}.blade.php`, `tests/Feature/Admin/BulkShipmentTest.php` | Iris |
| 2026-08-27 | **Bulk courier booking and AWB printing (REQ-013).** Selection on the order list now drives three actions; booking goes through a quote-and-confirm screen rather than a one-click bulk charge, and the admin picks the courier service there. **Fixes a bug introduced by REQ-006:** since delivery moved to the store's weight table, `orders.courier_service_id` holds `weight-west`/`weight-east`, which is not an EasyParcel product — `ShipmentPayload::for()` was sending it as `service_id` and every booking would have been rejected. The service is now a required argument chosen at book time. Also fixed a route-ordering bug (`/orders/book` was being matched as `/orders/{order}` and 404ing) and a second strict-mode N+1 on the order list | `app/Http/Controllers/Admin/{Shipment,Order}Controller.php`, `app/Support/ShipmentPayload.php`, `app/Services/{ShipmentBooking,EasyParcel}Service.php`, `app/Models/Order.php`, `app/Enums/OrderStatus.php`, `routes/web.php`, `resources/views/admin/orders/{index,book,labels,show}.blade.php`, `tests/Feature/Admin/BulkShipmentTest.php` | Iris |
| 2026-08-27 | **Bulk "Move to Processing" on the order list (REQ-007).** Checkboxes with select-all, a per-row button and a bulk bar — all posting to ONE action, the row button with an array of one. Only `new_order` may move: `Pending` is unpaid, `NeedsReview` needs a decision rather than a tick, and the rest would be moving backwards. Implemented as a guarded `UPDATE` filtered on the current status, so an order that changed underneath simply is not matched and the affected-row count reports exactly what moved. A partial result always says how many were skipped and why. Works without JavaScript — every control is a real form input; the script only adds select-all, the count and the indeterminate state | `app/Enums/OrderStatus.php`, `app/Http/Controllers/Admin/OrderController.php`, `routes/web.php`, `resources/views/admin/orders/index.blade.php`, `public/js/app.js`, `tests/Feature/Admin/OrderProcessingTest.php` | Iris |
| 2026-08-27 | **Delivery priced from the store's own weight table (REQ-006).** Four admin-set figures — a first-kilo and a per-extra-kilo price for West and East Malaysia — replace the courier picker at checkout. Part kilos round UP, the way a courier bills. Zone is decided by state code: Sabah, Sarawak and Labuan are East. `ShippingRate` derives the charge server-side from the submitted address and the cart's own weight, so no service id or price is accepted from the browser at all — a stronger guarantee than the old re-quote-and-match. EasyParcel quotations remain, but only as the booking cost; the gap between the two is the store's margin and already shows on the order screen. Also pinned `ProductVariantFactory`'s weight, which was random across the 1 kg boundary and would have made every shipping assertion flaky | `app/Support/ShippingRate.php`, `app/Http/Controllers/{Checkout,Shipping}Controller.php`, `app/Http/Requests/SettingRequest.php`, `app/Http/Controllers/Admin/SettingController.php`, `resources/views/admin/settings.blade.php`, `resources/views/storefront/checkout.blade.php`, `tests/Feature/ShippingRateTest.php` | Iris |
| 2026-08-27 | **Storefront redesign.** Custom theme on the already-vendored Bootstrap — no new dependency, no build step, no webfont (display face is a system serif). New `public/css/storefront.css` is storefront-only, so the admin's `app.css` can no longer be reached from a shop change. Four interactions added, all progressive enhancement: live variant picker (upgrades a `<select>` that still works), add-to-cart via fetch (same route, same code path, JSON or redirect by `expectsJson()`), product gallery (new `product_images` table; `products.image_path` keeps its meaning as the cover), and filter/sort/search (a plain GET form). Fixed on the way: an N+1 that strict mode caught in `coverUrl()`, a `HAVING` clause SQLite rejects, and courier names rendered into the checkout via `innerHTML` | `public/css/storefront.css`, `public/js/app.js`, `resources/views/layouts/storefront.blade.php`, `resources/views/storefront/*`, `resources/views/components/product-card.blade.php`, `app/Models/Product{,Image}.php`, `app/Http/Controllers/{Home,Product,Cart}Controller.php`, `database/migrations/2026_08_27_180000_*`, `tests/Feature/StorefrontThemeTest.php` | Iris |
